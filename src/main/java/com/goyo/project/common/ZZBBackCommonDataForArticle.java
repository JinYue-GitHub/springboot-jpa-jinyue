...
public class ZZBBackCommonDataForArticle{
	...
	public ZZBBackCommonDataForArticle() {
		super();
	}
		...
		if(StringUtils.isNoneBlank(pageNum)&&StringUtils.isNoneBlank(pageSize)){
			...
			page = baserepository.findAll(new Specification<Article>() {
	            ...
	            public Predicate toPredicate(Root<Article> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
					...
					Predicate sourceOrganizationp = null;
					Predicate namep = null;
	                Predicate typeIdsp = null;
	                String sourceOrganization = article.getSourceOrganization();
	                String name = article.getName();
	                String typeIds = article.getTypeIds();
	                if(null != article && !StringUtils.isEmpty(sourceOrganization)) {
	                	sourceOrganizationp = cb.equal(root.<String> get("sourceOrganization"), sourceOrganization);
	                }
					...
					if(null != sourceOrganizationp) query.where(sourceOrganizationp);
		...
		if(StringUtils.isNoneBlank(pageNum)&&StringUtils.isNoneBlank(pageSize)){
			...
			page = baserepository.findAll(new Specification<Article>() {
	            ...
	            public Predicate toPredicate(Root<Article> root, CriteriaQuery<?> query, CriteriaBuilder cb) {
					...
					if(Optional.ofNullable(sourceOrganizationp).isPresent()) query.where(sourceOrganizationp);
